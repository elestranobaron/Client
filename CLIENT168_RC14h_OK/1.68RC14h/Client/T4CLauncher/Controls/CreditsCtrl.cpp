...
void CCreditsCtrl::OnPaint() 
{
	...
	if(m_pBackgroundPaint!=NULL)
		(*m_pBackgroundPaint)(&memDC,m_rcClient,m_bIsScrolling||bFirstDraw,m_dwBackgroundPaintLParam,m_pBitmapBack);
	...
	{
      CRect rect = m_rcClient;
      rect.InflateRect(0,-5,0,-5);
      DrawTransparentBitmap(&m_bmpNormal,&memDC,m_crInternalTransparentColor,CRect(0,5,rect.right,min(m_nBitmapHeight-m_nCurBitmapOffset,rect.bottom)),CRect(0,m_nCurBitmapOffset,0,0));
		if(m_nBitmapHeight-m_nCurBitmapOffset < rect.bottom)
			DrawTransparentBitmap(&m_bmpNormal,&memDC,m_crInternalTransparentColor,CRect(0,m_nBitmapHeight-m_nCurBitmapOffset,rect.right,rect.bottom),CRect(0,0,0,0));
      /*
		DrawTransparentBitmap(&m_bmpNormal,&memDC,m_crInternalTransparentColor,CRect(0,0,m_rcClient.right,min(m_nBitmapHeight-m_nCurBitmapOffset,m_rcClient.bottom)),CRect(0,m_nCurBitmapOffset,0,0));
		if(m_nBitmapHeight-m_nCurBitmapOffset < m_rcClient.bottom)
			DrawTransparentBitmap(&m_bmpNormal,&memDC,m_crInternalTransparentColor,CRect(0,m_nBitmapHeight-m_nCurBitmapOffset,m_rcClient.right,m_rcClient.bottom),CRect(0,0,0,0));
         */
...
void CCreditsCtrl::DrawBackground(CDC *pDC, RECT rect, BOOL bAnimate, DWORD lParam,CBitmap *pBitMap)
{
...
{
	{
		...
		for(int i = 0; i < LinkElements.GetSize(); i++) // DaP ajout de int
		{
