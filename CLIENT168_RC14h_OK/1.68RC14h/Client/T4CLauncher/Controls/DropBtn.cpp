...
void CListWnd::OnSelectionChange()
{
	...
	m_pParent->SendMessage(WM_SELECTION_CHANGED, m_pDropList->GetCurSel(), 0);
}
...
void CDropButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct )
{
   ...
   bitmap.CreateCompatibleBitmap(pDC,ButtonRect.Width(),ButtonRect.Height());
   ...
   if(m_bTrans)
   {
      if(m_bColorBack)
      {
         memDC.FillRect(ButtonRect, &CBrush(m_crBack));
      }
      else
      {
         if(m_BGBitmap.m_hObject != NULL)
         {
            ...
            {
               pDC->SelectPalette(&m_BGBitmapPalette,FALSE);
               pDC->RealizePalette();
               memDC.SelectPalette(&m_BGBitmapPalette,FALSE);
            }
            memDC.FillRect(ButtonRect, &CBrush(RGB(255,255,255)));
         }
      }
      ...
      //Dessine le Texte ainsi que le triangle...
      CFont* pOldFont = (CFont*)memDC.SelectObject(m_pTextFont);
      int	nOldBkMode		= memDC.SetBkMode(TRANSPARENT);
      COLORREF crOldColor;
      if(!m_bOver)
	      crOldColor  = memDC.SetTextColor(m_crTxtColor);
      else
         crOldColor  = memDC.SetTextColor(m_crTxtOverColor);
      ...
      if(!csText.Compare("Mm"))
         csText = "";
      int nYStart = (ButtonRect.Height() - cText.cy) / 2;
      CSize cTri  = pDC->GetTextExtent( "M", 1 );
      CPoint ptTri(ButtonRect.right - (cTri.cx + 5), nYStart + (cTri.cy / 2));
      DrawTriangle(&memDC, ptTri, cTri.cx);
      CRect rcText(5, 10, m_WindowRect.right - (cTri.cx + 10), m_WindowRect.bottom);
      //memDC.DrawText(csText, csText.GetLength(),rcText, DT_LEFT|DT_VCENTER|DT_END_ELLIPSIS|DT_SINGLELINE);

      BOOL _bIsDisabled = TRUE;
      memDC.DrawState(rcText.TopLeft(), m_WindowRect.Size(),
				(LPCTSTR)csText, (bIsDisabled? DSS_DISABLED:DSS_NORMAL),
				TRUE, 0, (CBrush*)NULL);
      ...
      pDC->BitBlt( ButtonRect.left, ButtonRect.top, ButtonRect.Width(), ButtonRect.Height(),
                 &memDC, ButtonRect.left,ButtonRect.top, SRCCOPY );
...
LONG CDropButton::OnSelectionChanged(UINT wParam, LONG lParam)
{
	...
	GetParent()->PostMessage(WM_SELECTION_CHANGED, m_uID, wParam);
	m_pDropWnd->m_pDropList->SetPreviousSelection(wParam);
	return 1;
}

void  CDropButton::SetSelection(UINT dwIndex)
{
   if ( dwIndex < (UINT)m_pDropWnd->m_pDropList->GetCount( ) )
   {
      m_pDropWnd->m_pDropList->SetCurSel(dwIndex);

      CString csText;
	   m_pDropWnd->m_pDropList->GetText(dwIndex, csText);
	   SetWindowText(csText);
	   RedrawWindow();
   }
}

BOOL CDropButton::bSetBGImageTransRes(BOOL bColor,COLORREF crBack,UINT dwCtrlID,int dwXPos,int dwYPos)
{
   m_bTrans = TRUE;
...
BOOL CDropButton::OnEraseBkgnd(CDC* pDC) 
{
	if(m_BGBitmap.m_hObject == NULL)
   {
