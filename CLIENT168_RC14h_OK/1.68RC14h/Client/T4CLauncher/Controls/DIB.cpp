...
int i,j,k;
	...
	for(int i=0; i < yNum; i++,ySrc++) // DaP ajout de int
	{
...
unsigned char *srcPtr,*destPtr;
	if(!colors)
	{
		for(int i=0,k=nSrcY,l=nYDest; i < nHeight; i++,k++,l++) // DaP ajout de int
		{
			if(k < 0 || l < 0)
			{
				continue;
...
int i,j;
unsigned char *ptr;	
	for(int i=0; i < height; i++) // DaP ajout de int
	{
		ptr = GetLinePtr(i);
		for(j=0; j < width; j++)
		{
...
{
int i;
	for(int i=0;i<256;++i) // DaP ajout de int
	{
...
unsigned char cols[2];
	...
	for(int i=0; i < h; i++) // DaP ajout de int
	{
		dPtr = GetLinePtr(i);
		sPtr = dib.GetLinePtr(i);
		for(j=0 ; j < w; j++,dPtr++)
...
unsigned char cols[16];
	...
	h = Height();
	memset(CachePtr,0,sizeof(CachePtr));
	for(int i=0; i < 16; i++) // DaP ajout de int
	{
...
int pSize=GetPaletteSize();
	for(int i=0; i < (unsigned int)pSize;i++) // DaP ajout de int
	{
		if(CachePtr[i])
		{
	...
	for(int i=0; i < (unsigned int)pSize; i++,pQuad++) // DaP ajout de int
	{
...
int nNum=0,i,j,w,d;
	...
	d = Height();
	memset(colors,0,256);
	for(int i=0; i < d; i++) // DaP ajout de int
	{
		ptr = GetLinePtr(i);
		for(j=0; j < w; j++,ptr++)
		{
...
RGBQUAD rgb;
	...
	memset(CachePtr,0,sizeof(CachePtr));
	for(int i=0; i <= 0x7FFF; i++) // DaP ajout de int
	{
