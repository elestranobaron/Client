...
void VisualObjectList::Create(void) 
{
    ...
    int Image = 0;
    for(int nmI=0;nmI<__OBJANIM_MOULIND_CNT;nmI++) // DaP ajout de int
    {
    ...
    Image = 0;
    for(int nmI=0;nmI<__OBJANIM_MOULING_CNT;nmI++) // DaP ajout de int
    {
...
#define SGOLD   "Gold sound"    
    ...
    ItemDragSounds.BindSound( SDEF, __OBJGROUP_FIRECAMP ); //BLBLBL : 25 mars 2009 ajoutÃ© un son au feu de camp
...
void VisualObjectList::Add(unsigned long ID, unsigned short Type, unsigned short AbsX, unsigned short AbsY, signed char Light, unsigned char Friend, unsigned long AttachID, char pHp, char defaultDirection, bool allowOutOfBound, long spellEffectID, long spellChildID ) {
    ...
    if (!AlreadyHere) {
		...
		//////////////////////////////////////////////////////////////////////////////////////////
		// DaP TileSet::GetInstance()->GetFPSFRAMING() au lieu de FRAMING
		//////////////////////////////////////////////////////////////////////////////////////////
        Object->nbSprite4Move = 1* TileSet::GetInstance()->GetFPSFRAMING();
        Object->nbSprite4Att = 1* TileSet::GetInstance()->GetFPSFRAMING();
...
void VisualObjectList::MovePlayer(signed char X, signed char Y) {
    ...
    for (int i = 0; i < m_animObjNbr; i++) // DaP ajout de int 
    {
...
void VisualObjectList::MoveObject2(Deplacement *D, BOOL Fast) {
    ...
    if (D->Type == 0 && (Object->OX != (D->NewX-Player.xPos) || Object->OY != (D->NewY-Player.yPos))) {
        ...
        if (Fast) {
			...
			Object->SpeedX = (signed char)(-TileSet::GetInstance()->GetMOVX() / Object->Slow)*Speed; // DaP TileSet::GetInstance()->GetMOVX() au lieu de MOVX
        } else if (Object->OX-(D->NewX-Player.xPos) < 0) {
            Object->SpeedX = (signed char)(TileSet::GetInstance()->GetMOVX() /Object->Slow)*Speed; // DaP TileSet::GetInstance()->GetMOVX() au lieu de MOVX
        }
        ...
        else if (Object->OY-(D->NewY-Player.yPos) > 0) Object->SpeedY = (signed char)(-TileSet::GetInstance()->GetMOVY() /Object->Slow)*Speed; // DaP TileSet::GetInstance()->GetMOVY() au lieu de MOVY
        else if (Object->OY-(D->NewY-Player.yPos) < 0) Object->SpeedY = (signed char) (TileSet::GetInstance()->GetMOVY() /Object->Slow)*Speed; // DaP TileSet::GetInstance()->GetMOVY() au lieu de MOVY
...
{
	MainObject.nbSprite4Move = 1 * TileSet::GetInstance()->GetFPSFRAMING(); // DaP ajout
	MainObject.nbSprite4Att = 1 * TileSet::GetInstance()->GetFPSFRAMING(); // DaP ajout
   // Draw all the object to Screen.
   Lock(1668);
   TFCObject *Object = NULL;
   bool NeverDraw = true;
               ...
               if (Object->Dynamic == Flag) {
                  ...
                  if (Object->Chose == 1 && MultiPass == 1 && !bNewInt && !childFound) {
                     ...
                     else if (Object->Type == __OBJGROUP_WOODEN_DOOR8_OPENED && MultiPass == 1) 
                     {
                        ...
                        GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + __OBJGROUP_FIRECAMP)->DrawSpriteN(((Object->OX+ViewWidth)<<5)+XCor+Object->MovX, ((Object->OY+ViewHeight)<<4)+YCor+Object->MovY, VsfFX); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        Object->Faces++;
                        if (Object->DrawOverlap)
                           Object->Overlap.DrawSprite(((Object->OX+ViewWidth)<<5)+XCor, ((Object->OY+ViewHeight)<<4)+YCor, Clip, NULL, NULL, lpBlitSurface);
                        //               Object->Dynamic = true;
                     ...
                     {
                        GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + Object->Type)->DrawSpriteN(((Object->OX+ViewWidth)<<5)+XCor+Object->MovX-10, ((Object->OY+ViewHeight)<<4)+YCor+Object->MovY, VsfFX); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        x = ((Object->OX+ViewWidth)<<5)+XCor+Object->MovX;
                        y = ((Object->OY+ViewHeight)<<4)+YCor+Object->MovY;
                        x += GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + Object->Type)->GetCX(); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        y += GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + Object->Type)->GetCY(); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                     ...
                     else if (Object->Type == __OBJLIGHT_CHANDELLE2  && MultiPass == 1) 
                     {
                        ...
                        MultiPass == 1) {
                        int n = (Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING())); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        if (n >= Object->Anim) {
                           n = 0;
                           if (!(rand() % 1000)) {
                              Object->Faces = 0;
                        ...
                        x = ((Object->OX+ViewWidth)<<5)+XCor+Object->MovX;
                        y = ((Object->OY+ViewHeight)<<4)+YCor+Object->MovY;
                        x += GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + Type)->GetCX(); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        y += GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + Type)->GetCY(); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        Object->Faces++;
                        if (Object->DrawOverlap)
                           Object->Overlap.DrawSprite(((Object->OX+ViewWidth)<<5)+XCor, ((Object->OY+ViewHeight)<<4)+YCor, Clip, NULL, NULL, lpBlitSurface);
                        World.Restore();
                        ...
                        VsfFX->dwFX |= FX_LEFTRIGHTMIRROR;
                        GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + __OBJGROUP_TORCHE)->DrawSpriteN(((Object->OX+ViewWidth)<<5)+XCor+Object->MovX+10, ((Object->OY+ViewHeight)<<4)+YCor+Object->MovY, VsfFX); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        x = ((Object->OX+ViewWidth)<<5)+XCor+Object->MovX;
                        y = ((Object->OY+ViewHeight)<<4)+YCor+Object->MovY;
                        x += GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + __OBJGROUP_TORCHE)->GetCX(); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        y += GetVSFObject((Object->Faces / (2 * TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + __OBJGROUP_TORCHE)->GetCY(); // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        lmTorchLight.MergeLightMap((x-80-65)>>1, (y-40)>>1);
                        Object->Faces++;
                        //               Object->Dynamic = true;
                        World.Restore();
               ...
               if (Object->Dynamic == Flag) 
               {
                  ...
                  if (!bNewInt && !childFound) 
                  {
                         ...
                         Object->Type == __OBJANIM_MOULING)    && MultiPass == 1) 
                     {
                        ...
                        int dwLoopFrame;
                        //if(Object->Type == __OBJANIM_FONTAIN_02)
                           dwLoopFrame = (Object->Faces/(2* TileSet::GetInstance()->GetFPSFRAMING()) % Object->Anim) + Object->Type; // DaP Faces/(2*TileSet::GetInstance()->GetFPSFRAMING()) au lieu de Faces/2
                        //else
...
void VisualObjectList::DrawObjectAnimOverLap(int XCor, int YCor, LPDIRECTDRAWSURFACE lpBlitSurface) 
{
   ...
   int x, y, i; // DaP ajout de , i
   
   XCor -= 48;
   YCor += - 24 + 37;

   for(i=0;i<m_sAnimOverlap.size();i++) // DaP int retiré
   {
