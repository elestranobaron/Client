...
//#import "WebPatch.Exe" // DaP désactivation
// Include Files
#include <windows.h>
#include <shlobj.h>
#include "App.h"
...
#include "Disp.h"
#include "Ddraw.h"
//#include "Effect.h" // steph désactivation
#include <eh.h>
#include "ExpFltr.h"
#include "Fading.h"
#include "NewFont.h"
	  ...
	  case WM_SYSCOMMAND:
		  {
			  if(wParam == SC_MINIMIZE)
			  {
				  ...
				  //g_Var.tWaitFPSTime  = 1000/Custom.NMFPSValue;//BLBLBL on garde le même taux de FSP tout le temps // steph désactivation
				  //}
			  }
			  else// if(wParam == SC_MINIMIZE)
			  {
				  g_Var.minimizeState = false;
				  //g_Var.tWaitFPSTime = 1000/Custom.NMFPSValue; // steph désactivation
			  }
		  }
		  break;
 ...
 )
{
	...
	Custom.gWebPatchEnabled = false;//pConfig->WebpatchEnable;        
#else
	Custom.gWebPatchEnabled = true;//pConfig->WebpatchEnable; // steph ATTENTION à ne pas false en production !
#endif
	...
	if(dwQuitCnt>=50)
	{
		...
		RemoveFontResource("t4cbeaulieuxv2.ttf"); // steph Fonts\\ retiré
		exit(5678);
...
//CV2Sprite    Connect_D_Anim[36]; // steph désactivation
//CV2Sprite    Connect_Title;
CV2Sprite    Connect_Title2;
/* CV2Sprite    Connect_D;
CV2Sprite    Connect_V; */ // steph désactivation
...
UINT WINAPI SocketFunction(LPVOID pParam) 
{
	...
	while (NotRegister && SocketState == 2) 
	{
		if (timeGetTime() - LastTry > 1000 && NotRegister) 
		{
			LastTry = timeGetTime();
			TryNb++;
			if (TryNb == 4)//réduction du nombre d'essais à 5 (5 secondes) ça permet de rééssayer plus rapidement. // steph 4 au lieu de 5
			{
				DlgState = 4;
			}
...
void InitializeTFCData(void) 
{
	...
	*/
	...
	char Temp[100];
	UINT i; // DaP ajout
	for (i = 0; i < strlen(ServerIP); i++) { // DaP UINT retiré
        Temp[i] = ServerIP[i];
	}
	Temp[i] = 0;
	...
	while(pLauncher->IsLuncherAvailable() && dwAction != 1)
	{
     ...
     try 
     {
		 ...
		 char strISPName[20];
		 sprintf(strISPName,"%d.PCX", g_Global.GetScreenH()); // steph ISP retiré // steph H ScreenH au lieu de ScreenW
	 ...
	 Sleep(1500);//on laisse le message did you know 2 secondes de plus à l'écran // steph 1500 au lieu de 2000
	...
	//////////////////////////////////////////////////////////////////////////////////////////
     ...
     World.SetWorldFileName   ("Game Files\\V2_Extension03.Map"); */ // steph désactivation
     World.CreateView();
...
void ParseText(char *pText) 
{
	...
	{ 
		...
		/* else if (!stricmp(szCmd, "!RUN") && Player.CanRunScripts && g_bRunCommand == FALSE) 
		{
			...
			_beginthread(RunCommandThread, 0, NULL ); */ // steph désactivation
			/*
		...
		{
			char *spellId = strtok(NULL, " "); */ // steph désactivation
			/*
...
void DrawFisrtLoadingText()
{
	...
	//strcat(Text, ": "); // steph désactivation
