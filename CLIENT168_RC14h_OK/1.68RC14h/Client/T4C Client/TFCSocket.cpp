...
//extern CV2Sprite    Connect_D_Anim[36]; // steph désactivation
extern CV2Sprite    Connect_Title2;
/* extern CV2Sprite    Connect_D;
extern CV2Sprite    Connect_V; */ // steph désactivation
...
const int MAXMOV = 20; // DaP ajout de int
...
UINT WINAPI DrawThread(LPVOID pParam) 
{
   ...
   while (!g_boQuitApp) 
   {
      ...
      {
         Move = true;
         DrawLastMoving = TileSet::GetInstance()->GetDONE(); // DaP TileSet::GetInstance()->GetDONE() au lieu de DONE
         World.Lock(115);
         Objects.Lock(1009);
    ...
    while (!g_boQuitApp) {
        ...
        if (WantPreGame && !bSendPreGame) 
        {
            ...
            try 
            {
               ...
               char strLOADName[20];
               sprintf(strLOADName,"%d.PCX", g_Global.GetScreenH()); // steph LOAD retiré // steph H ScreenH au lieu de ScreenW
        ...
        if (!WantPreGame) 
        {
            if ((timeGetTime() - Try) > 500 && !Move && !NeedRedraw) // steph 500 au lieu de 250
            {
                LOG << "* TST = TimeOut\r\n";
                Move = true;
            }
            ...
            if (!QuitFirstLoop) { // boucle principale du jeu ?
				g_Var.tWaitFPSTime = 1000 / TileSet::GetInstance()->GetFPS(); // DaP ajout
				...
				//ChatterUI *chatterUI = ChatterUI::GetInstance(); // steph désactivation
               ...
               /* if (
					...
					// steph ajout
					/////////////////////////
				   if (((ChestUI::GetInstance()->IsShown()) || (TradeUI::GetInstance()->IsShown())))
                ...
                ManageAutoMove(KBuffer); // steph activation
...
UINT WINAPI MouseActionThread(LPVOID pParam){
    while( !g_boQuitApp ){
        ...
        Sleep( 25 ); // steph 25 au lieu de 50
    }
    return 0;
}
...
void TFCSocket::MenuThread(void) 
{
   ...
   {
      g_App.LockMainThread();
      Sleep(20); // steph 20 au lieu de 25
      ...
      int dwOffset = 20;
      if(g_Global.GetScreenW() == 1280) // steph 1024 au lieu de 800 // steph 1280 au lieu de 1280
         dwOffset = 40;
     ...
     //       DXDFlip();
         ...
         case TFC_MENU: //OKOKOK
         {
            ...
            DXDGetDC(&hdc, 36);
            int dwTPos = 0;
            for(int ib=0;ib<4;ib++) // DaP ajout de int
            {
               if(ib == 0)
                  dwTPos = 2;
               else if(ib == 1)
         ...
         case TFC_CHOOSE_SEXX: 
         {
            ...
            if (TFC_State == TFC_CHOOSE_PLAYER)
            {
               ...
               int dwTPos = 0;
               for(ib=0;ib<3;ib++) // DaP int retiré
               {
                  if(ib == 0)
                     dwTPos = 105;    //entrer
                  else if(ib == 1)
              ...
              switch(Type) 
              {
                  ...
                  case TFCGetPlayerList: 
                  {
                      ...
                      if (CreateFlag) 
                      {
                          ...
                          /* for(int i=0;i<36;i++) // steph ajout de int
                      ...
                      for (int i = 0; i < NbOfPlayer; i++) // steph ajout de int
                      {
         ...
         case TFC_CHOOSE_RACE: 
            {
               ...
               //dessine la liste des players....
               for (int i = 0; i < 2; i++) // DaP ajout de int
               {
         ...
         case TFC_MAKE_REROLL: 
            {
               ...
               int dwSelectedZone = -1;
			   int ib; // DaP ajout
               for(ib=0;ib<2;ib++) // DaP int retiré
               {
                  if(IsOnButton(Chose,x,y,&PS_Btn_N,dwXBtn,dwYBtn))
				  {
                     dwSelectedZone = ib;
...
void ManageAutoMove(char* strKeyInput)
{
   if(strKeyInput[DIK_INSERT] && !NMAutoMove) // steph ajout
	   /* if (strKeyInput[DIK_LCONTROL] && strKeyInput[DIK_LSHIFT] && !NMAutoMove)
   {
      ...
      if(NMAutoDir > 0) */ // steph désactivation
      {
         NMAutoMove = true;
         ChatterUI::GetInstance()->AddBackscrollItem("","AutoMove enabled.",RGB(255,255,255),true); // steph AutoMove enabled. au lieu de Starting AutoMove...
   ...
   else if(NMAutoMove)
   {
      ...
      /* if(/*strKeyInput[DIK_LCONTROL] ||*/ /*strKeyInput[DIK_NUMPAD5] ||
         ...
         strKeyInput[DIK_F9 ] || strKeyInput[DIK_F10] || strKeyInput[DIK_F11] || strKeyInput[DIK_F12]    ) */ // steph désactivation
		  if (strKeyInput[DIK_DELETE]) // steph ajout
      {
         NMAutoMove = false;
         NMAutoDir  = 0;
		 ChatterUI::GetInstance()->AddBackscrollItem("", "AutoMove disabled.", RGB(255, 255, 255), true); // steph AutoMove disabled. au lieu de Stopping AutoMove...
      }
      else
      {
         if(strKeyInput[DIK_DOWN] && strKeyInput[DIK_LEFT])
