...
Global::Global()
{
   ...
   sprintf(m_ZoneFile,"%s%s",m_strPath,"\\gamefiles\\Zone_Map.dat"); // steph gamefiles au lieu de Game Files
...
ClientConfig *Global::ReadClientConfig()
{
   ...
   if(!pf1)
   {
      ...
      //m_ClientConfig.dwNMFPSValue     = 17; // steph désactivation
   ...
   else
   {
      ...
      else
      {
         ...
         //fread(&m_ClientConfig.dwNMFPSValue    ,4,1,pf1); // steph désactivation
...
void Global::WriteClientConfig()
{
   ...
   if(pf1)
   {
      ...
      //fwrite(&m_ClientConfig.dwNMFPSValue    ,4,1,pf1); // steph désactivation
