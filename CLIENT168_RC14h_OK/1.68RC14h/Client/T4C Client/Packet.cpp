...
static const int TFC_MENU						= 5; // DaP ajout de int
   ...
   }
   {
        ...
        case 50: {//RQ_GetChatterUserList
			...
			for (j = 0; j < wUserQuantity; j++) {
				GET_STRING( szUser );
				GET_STRING( szTitle );
				GET_STRING(szGuild); // steph ajout
				...
				user.guild = reinterpret_cast< const char * >(szGuild); // steph ajout
      ...
      case __SPELL_FIREBALL_CIRCLE:
		  {
			  ...
			  static int iAngle = 0; // DaP ajout de int
   ...
   // Arrow hit
    ...
    case RQ_OpenURL:{
		...
		if( wLen > 0 ){
			...
			strcpy(lpbText, "http://");
			int wI; // DaP ajout
			for( wI = 0; wI < wLen; wI++ ){ // DaP int retiré
...
#define STAT_MAX_MANA       26 
    ...
    case 10001: {
        ...
        case TFCInGame: {
            ...
            */
			...
			}
			{
				...
				SEND_PACKET( Send );
				ChatterUI::GetInstance()->ClearBackScroll(); // steph ajout
			}
        ...
        case TFCQueryPlayerPosition: { 
			...
			while (NeedRedraw) Sleep(20); // steph 20 au lieu de 10
			//							Msg->Get((short *)&GeX);
			//							Msg->Get((short *)&GeY);
			Msg->Get((short *)&X);
			Msg->Get((short *)&Y);
