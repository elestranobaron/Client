...
#define RQ_GetPvpRanking				131 */ // asteryth ajout
   ...
   switch (PacketID) { 
        ...
        case RQ_GetPvpRanking: SETACK( 500, 3 ); // asteryth ajout
...
void PacketCenter::SetAlive(DWORD dwT) {
   dwTime = (timeGetTime() - (120000-dwT));//BLBLBL 120000=>60000 07/12/2010 ==> 120000
}


BOOL PacketCenter::isHalf(void) {
	DWORD dwTest = timeGetTime();
	if ((dwTime < dwTest) && ((dwTest - dwTime) > 4000)) {//1500 //BLBLBL : c'est ça qui fait afficher la Prise rouge, je le met à la moitiée du temps qui considère qu'on est out après.//07/12/2010 je remet 1500 08/12/2010 je met 3000 // steph 4000 au lieu de 3000
		return FALSE;									  
	}
