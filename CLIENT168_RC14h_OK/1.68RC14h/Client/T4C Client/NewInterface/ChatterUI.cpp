...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    int dwXPos = (g_Global.GetScreenW()-768)/2; // steph 768 au lieu de 576
    ...
    X_Button.SetPos( FramePos( dwXPos+768-28, dwYPos+0, dwXPos+768-4, dwYPos+0+24 ) ); // steph 768 au lieu de 576
    ...
    backChatCC.SetPos( FramePos( dwXPos, dwYPos, dwXPos+768, dwYPos+368 ) ); // steph 768 au lieu de 576
    ...
    selectedChannel = "Main"; // steph Main au lieu de English
    ...
    userList.SetPos( FramePos( dwXPos+227, dwYPos+59, dwXPos+742, dwYPos+284 ) ); // steph 742 au lieu de 552
    userList.SetupList( 3, 16, FramePos( 0, 0, 5, 5 ) ); // steph 3 au lieu de 2
    userList.SetColumnSize( 0, 146 ); // steph 146 au lieu de 144
    userList.SetColumnSize( 1, 185 ); // steph 185 au lieu de 146
	userList.SetColumnSize(2, 0); // steph ajout
    userList.SetScrollRegions(
        FramePos( dwXPos+720, dwYPos+61 , dwXPos+743, dwYPos+81 ), // steph 720 au lieu de 530 et 743 au lieu de 553
        FramePos( dwXPos+720, dwYPos+261, dwXPos+743, dwYPos+282 ), // steph 720 au lieu de 530 et 743 au lieu de 553
        FramePos( dwXPos+720, dwYPos+82 , dwXPos+743, dwYPos+253 ) // steph 720 au lieu de 530 et 743 au lieu de 553
    );
    userListingSelectionGraphPatch.SetSpriteId( "CUI_ChatSelectUser" );
    userListingSelectionGraphPatch.SetPos( FramePos( 0, 0, 488, 16 ) ); // steph 488 au lieu de 300
    ...
    int dwOffset = 985+8-(g_Global.GetScreenW()-287); // steph 985 au lieu de 729
    dwOffset*=-1;



    textInput.EnableCursor( true );
    textInput.SetPos( FramePos( 40,g_Global.GetScreenH()-28, dwOffset+8+1016-48-48, g_Global.GetScreenH()-8 ) ); // steph 1016 au lieu de 737
    ...
    minimizedGraphHL.DrawAlpha(true); */ // steph désactivation
	...
	//////////////////////////////////////////////////////////////////////////////////////////
	minimizedGraph.SetSpriteId("GUI_backChat");
	minimizedGraph.SetPos(FramePos(dwOffset+0, g_Global.GetScreenH() - 150, dwOffset+0 + 1280, g_Global.GetScreenH()));
	minimizedGraph.DrawAlpha(true);
	minimizedGraphHR.SetSpriteId("GUI_backHRChat");
	minimizedGraphHR.SetPos(FramePos(dwOffset+0, g_Global.GetScreenH() - 150, dwOffset+0 + 1280, g_Global.GetScreenH()));
	minimizedGraphHR.DrawAlpha(true);
	minimizedGraphHL.SetSpriteId("GUI_backHLChat");
	minimizedGraphHL.SetPos(FramePos(dwOffset+0, g_Global.GetScreenH() - 300, dwOffset+0 + 996, g_Global.GetScreenH()));
    ...
    minimizedGraphHHHT.DrawAlpha(true); */ // steph désactivation
	...
	//////////////////////////////////////////////////////////////////////////////////////////
	minimizedGraphHHHB.SetSpriteId("GUI_backHHHBChat");
	minimizedGraphHHHB.SetPos(FramePos(dwOffset+0, g_Global.GetScreenH() - 300, dwOffset+0 + 996, g_Global.GetScreenH()));
	minimizedGraphHHHB.DrawAlpha(true);

	minimizedGraphHHHT.SetSpriteId("GUI_backHHHTChat");
	minimizedGraphHHHT.SetPos(FramePos(dwOffset+0, g_Global.GetScreenH() - 292, dwOffset+0 + 996, g_Global.GetScreenH() - 300));
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    if( !channelID.empty() ){
		...
		// DaP ajout
		//////////////////////////////////////////////////////////////////////////////////////////
		std::list<RegChannel>::iterator pChannelTrouve =
			find_if(regChannels.begin(), regChannels.end(), [&](RegChannel const& p) { return p.channelID == regChannel.channelID; });
		
		if (pChannelTrouve != regChannels.end())
		{
			(*pChannelTrouve).pwd = pwd;
			(*pChannelTrouve).color = color;
			(*pChannelTrouve).bActive = bEnable;
		}
		else
        ...
        regChannels.push_back( regChannel ); // DaP push_back au lieu de insert
   ...
   if( textInputState == SendToPage )
   {
    ...
    if( analyzeCh )
    {
        ...
        {   
			...
			// steph ajout
			//////////////////////////////////////////////////////////////////////////////////////////
			if (textInput.GetText() == "!guide")
			{
				AddBackscrollItem("", g_GUIDELocalString[1], RGB(255, 255, 255), false);
				textInput.SetText("");
				return true;
			}
			else if (textInput.GetText() == "!1.25")
			{
				AddBackscrollItem("", g_GUIDELocalString[2], RGB(255, 255, 255), false);
				textInput.SetText("");
				return true;
			}
               ...
               case SendToPage:
               {
                  ...
                  if(!pConfig->dwAfkStatus)
                  {
                     ...
                     if (!pOption->bPageEnable) 
                     {
						...
						ChatterUI::GetInstance()->AddBackscrollItem("", g_LocalString[100], RGB(255, 255, 255), true);
                     } 
                     else 
                     {
                        ...
                        sending << (char *)textInput.GetText().c_str();
							...
							if (!ChatterUI::GetInstance()->IsUserIgnored(pageNameEdit.GetText().c_str()))
                        {
                           SEND_PACKET( sending );
                        } 
                        else 
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    list< RegChannel >::iterator j; // DaP list au lieu de set
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    for( i = channelUsers.begin(); i != channelUsers.end(); i++ ){
		...
		userUI = new UserUI;
		if ((*i).listenState) {
			userUI->SetText((*i).guild.c_str(), RGB(255, 255, 255), 640, 0, 0);
		}
		else {
			userUI->SetText((*i).guild.c_str(), RGB(180, 180, 180), 640, 0, 0);
		}
		userUI->SetCenter(false);
		userUI->SetPos(FramePos(0, 0, userUI->GetWidth(), userUI->GetHeight()));
		userUI->userID = (*i).guild;
		userList.AddColumnItem(2, userUI);
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    list< RegChannel >::iterator c; // DaP list au lieu de set
...
void ChatterUI::Draw( V2SPRITEFX *vsfFX )
{
   ...
   fNewGuiBtnFont->DrawFont(hdc, dwXPos+TxtOff(fNewGuiBtnFont,hdc,strBtnTexte ,101)+ 332+1, dwYPos+0+1 , RGB(0, 0, 0)    , 0, strBtnTexte); // steph 332 au lieu de 237
   fNewGuiBtnFont->DrawFont(hdc, dwXPos+TxtOff(fNewGuiBtnFont,hdc,strBtnTexte ,101)+ 332  , dwYPos+0   , g_Global.GetBtnTCE(), 0, strBtnTexte); // steph 332 au lieu de 237
   ...
   fNewGuiFont->DrawFont(hdc, dwXPos+TxtOff(fNewGuiFont,hdc,strBtnTexte ,71)+ 430, dwYPos+41 , g_Global.GetBtnTCE()  , 0, strBtnTexte); // steph 430 au lieu de 412
    ...
    fNewGuiFont->DrawFont(hdc, dwXPos + TxtOff(fNewGuiFont, hdc, strBtnTexte, 71) + 603, dwYPos + 41, g_Global.GetBtnTCE(), 0, strBtnTexte);
