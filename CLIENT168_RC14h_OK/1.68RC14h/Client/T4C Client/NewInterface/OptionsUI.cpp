...
OptionsUI::OptionsUI( void ) 
 :
	...
	normalCPUButton(normalCpuButtonEvent), // steph ajout
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    int dwYPos = (g_Global.GetScreenH()-150-400)/2; // steph 400 au lieu de 368
    if(dwYPos <0)
       dwYPos =0;

    
    SetPos( FramePos( dwXPos, dwYPos, dwXPos+512, dwYPos+400 ) ); // steph 400 au lieu de 368
    SetHelpId( this, 387 );

    backOption.SetSpriteId( "GUI_BackOption" );    
    backOption.SetPos( FramePos( dwXPos, dwYPos, dwXPos+512, dwYPos+400 ) ); // steph 400 au lieu de 368
	...
	// steph activation
	//////////////////////////////////////////////////////////////////////////////////////////
    cdMusic.SetPos( FramePos( dwXPos+228, dwYPos+129 , dwXPos+243, dwYPos+144 ) ); // steph 228 au lieu de 233 et 243 au lieu de 248 et 129 au lieu de 132 et 144 au lieu de 147
    /* cdMusic.SetSpriteId   ( "CheckONLightUP" );
    cdMusic.SetDownUI     ( "CheckONLightDown" );
    cdMusic.SetHighlightUI( "CheckONLightHUP" ); */ // steph désactivation
    cdMusicEvent.SetUI( this );
    SetHelpId( cdMusic, 391 );

    disabledCdMusic.SetPos( FramePos( dwXPos+228, dwYPos+129 , dwXPos+243, dwYPos+144 ) ); // steph 228 au lieu de 233 et 243 au lieu de 248 et 129 au lieu de 132 et 144 au lieu de 147
    /* disabledCdMusic.SetSpriteId    ( "CheckOFFLightUP" );
    disabledCdMusic.SetDownUI      ( "CheckOFFLightDown" );
    disabledCdMusic.SetHighlightUI ( "CheckOFFLightHUP" ); */ // steph désactivation
    ...
    quitButton.SetPos ( FramePos( dwXPos+177, dwYPos+300, dwXPos+177+60, dwYPos+300+32 ) ); // steph 177 au lieu de 274
    ...
    cancelButton.SetPos ( FramePos( dwXPos+253, dwYPos+300, dwXPos+253+60, dwYPos+300+32 ) ); // steph 253 au lieu de 174
    ...
    lowCPUButton.SetPos ( FramePos( dwXPos+361, dwYPos+300, dwXPos+361+60, dwYPos+300+32 ) ); // steph 361 au lieu de 437
	...
	// steph ajout
	//////////////////////////////////////////////////////////////////////////////////////////
	normalCPUButton.SetPos(FramePos(dwXPos + 437, dwYPos + 300, dwXPos + 437 + 60, dwYPos + 300 + 32));
	normalCPUButton.SetSpriteId("GUI_ButtonUp");
	normalCPUButton.SetDownUI("GUI_ButtonDown");
	normalCPUButton.SetHighlightUI("GUI_ButtonHUp");
	normalCpuButtonEvent.SetUI(this);
	SetHelpId(normalCPUButton, 600);
	...
	AddChild(&normalCPUButton); // steph ajout
...
void OptionsUI::Draw(V2SPRITEFX *vsfFX)
{
   ...
   // Draw les texte...
   char strBtnTexte[25]; // steph 25 au lieu de 20
   // Draw Box Header
   ...
   sprintf(strBtnTexte  ,"%s",g_GUILocalString[46]); // steph activation
   //sprintf(strBtnTexte  ,"www.AboT4C.com"); // steph désactivation
   ...
   fNewGuiBtnFont->DrawFont(hdc, dwXPos+TxtOff(fNewGuiBtnFont,hdc,strBtnTexte ,60)+ 177, dwYPos+300+4 , RGB(0, 0, 0)    , 0, strBtnTexte); // steph 177 au lieu de 274

   sprintf(strBtnTexte, "%s", g_GUILocalString[11]);
   fNewGuiBtnFont->DrawFont(hdc, dwXPos + TxtOff(fNewGuiBtnFont, hdc, strBtnTexte, 60) + 253, dwYPos + 300 + 4, RGB(0, 0, 0), 0, strBtnTexte); // steph 253 au lieu de 174

   sprintf(strBtnTexte  ,"%s",g_LocalString[580]);
   fNewGuiBtnFont->DrawFont(hdc, dwXPos+TxtOff(fNewGuiBtnFont,hdc,strBtnTexte ,60)+ 361, dwYPos+300+4 , RGB(0, 0, 0)    , 0, strBtnTexte); // steph 361 au lieu de 437
   ...
   // steph ajout
   //////////////////////////////////////////////////////////////////////////////////////////
   sprintf(strBtnTexte, "%s", g_LocalString[601]);
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    /* g_GameMusic.Stop();
    g_GameMusic.Reset(); */ // steph désactivation
    
    g_SaveGame.GetOptionsParam()->bEnable32FPS = !g_SaveGame.GetOptionsParam()->bEnable32FPS; // steph Enable32FPS au lieu de UseCD

    if( g_SaveGame.GetOptionsParam()->bEnable32FPS ){ // steph Enable32FPS au lieu de UseCD
        ui->cdMusic.Show();
        ui->disabledCdMusic.Hide();
		ChatterUI::GetInstance()->AddBackscrollItem("", "Toggle FPS.", RGB(255, 0, 0), true); // steph ajout
    }else{
        ui->cdMusic.Hide();
    ...
    //g_GameMusic.LoadNewSound(); // steph désactivation
}
...
// steph ajout
//////////////////////////////////////////////////////////////////////////////////////////
void OptionsUI::NormalCpuButtonEvent::LeftClicked(void)
{
	OptionsUI *ui = static_cast< OptionsUI * >(GetUI());

	OptionParam *pOption = g_SaveGame.GetOptionsParam();

	pOption->bLightHightGraph = 0;
	pOption->bHightGraphEffect = 1;

	pOption->bAnimatedWater = 1;
	g_Global.ResetAnimWater();
	World.Lock(0);
	Objects.Lock(0);
	World.SetPosition(Player.xPos, Player.yPos, Player.World, TRUE, FALSE);
	...
	pOption->bDisplayStatus = 1;
	pOption->bDisplayXPBarText = 1;
	pOption->bEnableDisplayGold = 1;
	pOption->bDisplayMacroFullScreen = 1;
	pOption->bLockResize = 0;
	pOption->bLockTarget = 0;

	pOption->bHighFont = 0;
	ChatterUI::GetInstance()->SetMinimizedHight(ChatterUI::GetInstance()->IsMinimizedHight());

	pOption->bEnableZoom = 1;
	g_Global.SetZoomStatus(0);
	...
	pOption->bShowWeatherEffects = 1;
	pOption->bOldStatBar = 0;
...
void OptionsUI::UpdateGraphList()
{
   ...
   // steph ajout
   //////////////////////////////////////////////////////////////////////////////////////////
   sprintf(strParam, "%s", g_GUILocalString[226]);
   AddGraphparameters(strParam, 20, pOption->bEnableDisplayGold);
   ...
   AddGraphparameters(strParam,15, pOption->bHyperChat); */ // steph désactivation
...
void OptionsUI::GraphListEvent::LeftClicked( void )
{
    ...
    else if (col == 1)
    {
        ...
        switch(optUI->dwID)
        {
		   ...
		   case 20:pOption->bEnableDisplayGold		= !pOption->bEnableDisplayGold		; break; // steph ajout
