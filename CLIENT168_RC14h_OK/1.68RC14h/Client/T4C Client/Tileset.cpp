...
const int Square = 36;//25; // DaP ajout de int
...
void TileSet::LoadVirtualMap(int xLoad, int yLoad, int World) {
 ...
 // Load the Virtual Map into memory.
   ...
   RLE_Decompress((unsigned short *)Buffer2, (unsigned short *)Buffer, 128*128, 65536, LASTONE);
	...
	char Temp2[100];
	sprintf(Temp2, "gamefiles\\Map%d.Dat", World+1); // steph gamefiles au lieu de Game Files
...
void TileSet::Switch(void) {
   ...
   LOG << "(" << MovX << ", " << TileSet::GetInstance()->GetMOVX() << ", " << MovY << ", " << TileSet::GetInstance()->GetMOVY() << ", " << Done << ")\r\n"; // DaP TileSet::GetInstance()->GetMOVX() au lieu de MOVX et TileSet::GetInstance()->GetMOVY() au lieu de MOVY
   
    /* SmoothSource.left += (MovX*MOVX*Done);
	SmoothSource.top += (MovY*MOVY*Done);
	SmoothSource.right += (MovX*MOVX*Done);
	SmoothSource.bottom += (MovY*MOVY*Done); */ // steph désactivation
...
// DaP ajout
//////////////////////////////////////////////////////////////////////////////////////////
    SmoothSource.left += (MovX*TileSet::GetInstance()->GetMOVX()*Done);
	SmoothSource.top += (MovY*TileSet::GetInstance()->GetMOVY()*Done);
	SmoothSource.right += (MovX*TileSet::GetInstance()->GetMOVX()*Done);
	SmoothSource.bottom += (MovY*TileSet::GetInstance()->GetMOVY()*Done);
...
void TileSet::UpdateView(bool Draw, bool bNewInt, int xPos, int yPos) 
{
   if (Done2 && !bNewInt) 
   {
      ...
      SmoothSource2.bottom += MovY2*MOVY; */ // steph désactivation
	  
	  
	  //////////////////////////////////////////////////////////////////////////////////////////
	  // DaP ajout
	  //////////////////////////////////////////////////////////////////////////////////////////
      SmoothSource2.left   += MovX2*TileSet::GetInstance()->GetMOVX();
      SmoothSource2.top    += MovY2*TileSet::GetInstance()->GetMOVY();
      SmoothSource2.right  += MovX2*TileSet::GetInstance()->GetMOVX();
      SmoothSource2.bottom += MovY2*TileSet::GetInstance()->GetMOVY();
   ...
   if(g_SaveGame.GetOptionsParam()->bAnimatedWater)
      DrawWaterLevel(true,MovX2*(Done2)*TileSet::GetInstance()->GetMOVX(), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY(),lpPreLightSurface); // DaP

   Objects.Lock(0);
   Objects.Sort();

   Objects.DrawObjectAnim(MovX2*(Done2)*TileSet::GetInstance()->GetMOVX(), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY(), Draw, lpPreLightSurface, true, false, bNewInt); // DaP
   Objects.DrawObjectSelect(MovX2*(Done2)*TileSet::GetInstance()->GetMOVX(), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY(), Draw, lpPreLightSurface, true, false, bNewInt); // DaP
   Objects.DrawObjectShadow(MovX2*(Done2)*TileSet::GetInstance()->GetMOVX(), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY(), Draw, lpPreLightSurface, true, false, bNewInt); // DaP
   Objects.DrawObject(MovX2*(Done2)*TileSet::GetInstance()->GetMOVX(), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY(), Draw, lpPreLightSurface, true, false, bNewInt); // DaP
   int dwFound = Objects.FoundAnimListID(99999999);
   if(dwFound>=0)
   {
      Objects.DrawObjectAnimOverLapID(dwFound,true,MovX2*(Done2)*TileSet::GetInstance()->GetMOVX(), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY(),lpPreLightSurface); // DaP
   }
   ...
   if((Player.World == 0 || /* Player.World == 4 // steph désactivation || */ (Player.World == 3 && Player.yPos < 1536)) && m_wRainStatus != 0 && g_SaveGame.GetOptionsParam()->bShowWeatherEffects )
   {
	  //if(!bSnowZone)
	  {
		  V2SPRITEFX fx;
   ...
   if((Player.World == 0 || /* Player.World == 4 // steph désactivation || */ (Player.World == 3 && Player.yPos < 1536)) && m_wFogStatus != 0 && g_SaveGame.GetOptionsParam()->bShowWeatherEffects)
   {
   ...
   if (!bNewInt) 
   {
      ...
      if(dwZoomStatus >0)
      {
         ...
         Objects.Lock(0);
         Objects.DrawObjectSelect(MovX2*(Done2)*TileSet::GetInstance()->GetMOVX() +(xPos/2), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY() +(yPos/2), Draw, lpDXDirectDraw->GetBackBufferDirect(), true, true, false); // DaP
         Objects.DrawObjectShadow(MovX2*(Done2)*TileSet::GetInstance()->GetMOVX() +(xPos/2), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY() +(yPos/2), Draw, lpDXDirectDraw->GetBackBufferDirect(), true, true, false); // DaP
         Objects.DrawObject(MovX2*(Done2)*TileSet::GetInstance()->GetMOVX() +(xPos/2), MovY2*(Done2)*TileSet::GetInstance()->GetMOVY() +(yPos/2), Draw, lpDXDirectDraw->GetBackBufferDirect(), true, true, false); // DaP
...
// Make the map move to x,y position.
   ...
   if (!Done) 
   {
      ...
      if (MovX > 1 || MovY > 1) {
		...
		Done = TileSet::GetInstance()->GetDONE(); // DaP TileSet::GetInstance()->GetDONE()au lieu de DONE
      ...
      g_Global.ValidMapZonePosition(Player.xPos,Player.yPos);
		...
		for (j = 0; j < Done; j++) {
			SmoothSource.left   -= MovX*TileSet::GetInstance()->GetMOVX(); // DaP
			SmoothSource.top    -= MovY*TileSet::GetInstance()->GetMOVY(); // DaP
			SmoothSource.right  -= MovX*TileSet::GetInstance()->GetMOVX(); // DaP
			SmoothSource.bottom -= MovY*TileSet::GetInstance()->GetMOVY(); // DaP
		}
